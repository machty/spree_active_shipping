  ...
  class Calculator < ActiveRecord::Base
    module Usps
      class ExpressMailInternational < Calculator::Usps::Base
        ...
        def available?(order)
          # Override the default behavior in Usps::Base
          # which checks for
          # order.products.any? { |p| p.meta_keywords == "priority_mail_express_international_only" }
          # Remember: this was for a yoko album that could only be
          # priority_mail_express_international_only
          true
        end
